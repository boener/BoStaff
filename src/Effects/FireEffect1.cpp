#include "BoStaff.h"
#include "Effects/FireEffect1.h"
#include "EffectsConfig.h"

FireEffect1::FireEffect1(LEDController* ledController, int segmentLen) : 
  ledArray(nullptr), numLedsTotal(0), segmentLength(segmentLen), heat(nullptr), 
  cooling(FIRE1_COOLING), sparking(FIRE1_SPARKING), initialized(false), controller(ledController) {
  
  // Validate inputs
  if (!ledController) {
    Serial.println("ERROR: FireEffect1 created with invalid parameters");
    return;
  }
  ...
  if (heat) {
    ...
    Serial.println("FireEffect1 initialized with single-strip approach");
    
    // Log configuration settings from EffectsConfig.h
    Serial.print("Fire Effect 1 Config - Cooling: ");
    Serial.print(FIRE1_COOLING);
    Serial.print(", Sparking: ");
    Serial.print(FIRE1_SPARKING);
    Serial.print(", Heat Dissipation: ");
    Serial.println(FIRE1_HEAT_DISSIPATION);
  } else {
    Serial.println("ERROR: FireEffect1 failed to allocate heat arrays");
  }
}

FireEffect1::~FireEffect1() {
  if (heat) {
    delete[] heat;
...
bool FireEffect1::isInitialized() const {
  return initialized && heat != nullptr && ledArray != nullptr && controller != nullptr;
}

void FireEffect1::setCooling(uint8_t cool) {
  cooling = cool;
}

void FireEffect1::setSparking(uint8_t spark) {
  sparking = spark;
}

void FireEffect1::update() {
  // Safety check - make sure we have valid memory and initialization
  if (!isInitialized()) {
    static bool errorLogged = false;
    if (!errorLogged) {
      Serial.println("ERROR: FireEffect1 update called on uninitialized effect");
      errorLogged = true;
    }
    return;
  }
...
void FireEffect1::updateSegment(int segmentIndex) {
  ...
  for (int k = segmentLength - 1; k >= 2; k--) {
    heat[heatOffset + k] = (heat[heatOffset + k - 1] + 
                           heat[heatOffset + k - 2] + 
                           heat[heatOffset + k - 2]) / FIRE1_HEAT_DISSIPATION;
  }
  ...
  for (int j = 0; j < segmentLength; j++) {
    ...
    CHSV baseColorHSV = rgb2hsv_approximate(FIRE1_BASE_COLOR);
    
    // Adjust hue towards the blue base color
    // This makes it a blue fire while keeping the intensity dynamics
