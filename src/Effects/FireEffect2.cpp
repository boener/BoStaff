#include "BoStaff.h"
#include "Effects/FireEffect2.h"
#include "EffectsConfig.h"

FireEffect2::FireEffect2(LEDController* ledController, int segmentLen) : 
  ledArray(nullptr), numLedsTotal(0), segmentLength(segmentLen), heat(nullptr), 
  cooling(FIRE2_COOLING), sparking(FIRE2_SPARKING), initialized(false), controller(ledController) {
  
  // Validate inputs
  if (!ledController) {
    Serial.println("ERROR: FireEffect2 created with invalid parameters");
    return;
  }
  ...
  if (heat) {
    ...
    Serial.println("FireEffect2 initialized with single-strip approach");
    
    // Log configuration settings from EffectsConfig.h
    Serial.print("Fire Effect 2 Config - Cooling: ");
    Serial.print(FIRE2_COOLING);
    Serial.print(", Sparking: ");
    Serial.print(FIRE2_SPARKING);
    Serial.print(", Heat Dissipation: ");
    Serial.println(FIRE2_HEAT_DISSIPATION);
  } else {
    Serial.println("ERROR: FireEffect2 failed to allocate heat arrays");
  }
}

FireEffect2::~FireEffect2() {
  if (heat) {
    delete[] heat;
...
bool FireEffect2::isInitialized() const {
  return initialized && heat != nullptr && ledArray != nullptr && controller != nullptr;
}

void FireEffect2::setCooling(uint8_t cool) {
  cooling = cool;
}

void FireEffect2::setSparking(uint8_t spark) {
  sparking = spark;
}

void FireEffect2::update() {
  // Safety check - make sure we have valid memory and initialization
  if (!isInitialized()) {
    static bool errorLogged = false;
    if (!errorLogged) {
      Serial.println("ERROR: FireEffect2 update called on uninitialized effect");
      errorLogged = true;
    }
    return;
  }
...
void FireEffect2::updateSegment(int segmentIndex) {
  ...
  for (int k = segmentLength - 1; k >= 2; k--) {
    heat[heatOffset + k] = (heat[heatOffset + k - 1] + 
                           heat[heatOffset + k - 2] + 
                           heat[heatOffset + k - 2]) / FIRE2_HEAT_DISSIPATION;
  }
  ...
  for (int j = 0; j < segmentLength; j++) {
    // Get base heat color (standard yellow-red palette)
    CRGB color = HeatColor(heat[heatOffset + j]);
    
    // Convert to HSV to modify the hue
    CHSV hsvColor = rgb2hsv_approximate(color);
    
    // Get the base color as a CHSV
    CHSV baseColorHSV = rgb2hsv_approximate(FIRE2_BASE_COLOR);
    
    // Adjust hue towards the green base color
    // This makes it a green fire while keeping the intensity dynamics
    hsvColor.hue = baseColorHSV.hue;
